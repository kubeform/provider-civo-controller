...
// +kubebuilder:rbac:groups=volume.civo.kubeform.com,resources=attachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=volume.civo.kubeform.com,resources=attachments/status,verbs=get;update;patch

func (r *AttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Attachment")
...
func (r *AttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &volumev1alpha1.Attachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", volumev1alpha1.Attachment{}.APIVersion, volumev1alpha1.Attachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&volumev1alpha1.Attachment{}).
