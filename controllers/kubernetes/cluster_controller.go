...
// +kubebuilder:rbac:groups=kubernetes.civo.kubeform.com,resources=clusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kubernetes.civo.kubeform.com,resources=clusters/status,verbs=get;update;patch
...
func (r *ClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kubernetesv1alpha1.Cluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", kubernetesv1alpha1.Cluster{}.APIVersion, kubernetesv1alpha1.Cluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kubernetesv1alpha1.Cluster{}).
...
func (r *ClusterReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for Cluster")
			return nil
		}
		...
		resourceList := &kubernetesv1alpha1.ClusterList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Cluster")
			return nil
		}
