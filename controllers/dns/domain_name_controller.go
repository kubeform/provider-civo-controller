...
// +kubebuilder:rbac:groups=dns.civo.kubeform.com,resources=domainnames,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.civo.kubeform.com,resources=domainnames/status,verbs=get;update;patch

func (r *DomainNameReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainname", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainName")
...
func (r *DomainNameReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.DomainName{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.DomainName{}.APIVersion, dnsv1alpha1.DomainName{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.DomainName{}).
