...
// +kubebuilder:rbac:groups=dns.civo.kubeform.com,resources=domainrecords,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dns.civo.kubeform.com,resources=domainrecords/status,verbs=get;update;patch

func (r *DomainRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainrecord", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainRecord")
...
func (r *DomainRecordReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dnsv1alpha1.DomainRecord{}); err != nil {
			klog.Error(err, "unable to set up auditor", dnsv1alpha1.DomainRecord{}.APIVersion, dnsv1alpha1.DomainRecord{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dnsv1alpha1.DomainRecord{}).
